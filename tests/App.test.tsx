...
function testPropAndSlotComponent({ enabled, name, dataFoo, rows, header, footer, as } : { name : string, dataFoo : string, enabled : boolean, rows: { label: string, value: string }[], header: ReactNode, footer: ReactNode }) {
    return <article name={name} foo={dataFoo}>
        {header}
...
<div id="htx-app" data-testid="htx-app">
    ...
    <ui-custom></ui-custom>
        ...
        expect(root.querySelector('pre[data-is="ui-button"]')).toHaveTextContent('Test Button 1')
        expect(root.querySelector('pre[data-is="ui-custom"]')).toBeEmptyDOMElement()
    });
...
<div id="htx-app" data-testid="htx-app">
    <my-comp key="fo" enabled name="test" data-foo="baa" as="{div}" json-rows='[{ "label": "Foo", "value": "Baa" }, { "label": "Foo1", "value": "Baa1" }]'>
        ...
        expect(root.querySelector('article dl div:first-child dt')).toHaveTextContent('Foo')
        expect(root.querySelector('article dl div:last-child dd')).toHaveTextContent('Baa1')

        expect(root.querySelector('article h1')).toBeNull()
        expect(root.querySelector('article')).toHaveTextContent('HEADER')
        expect(root.querySelector('article footer h4')).toHaveTextContent('FOOTER!')
    })
});
